...
unsigned_long randomseed;                     /* Current random number seed. */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((unsigned_long) (ptr) & (unsigned_long) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((unsigned_long) (ptr) ^ (unsigned_long) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((unsigned_long) (otri).tri | (unsigned_long) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((unsigned_long) (otri).tri[6] | (unsigned_long) 2l)
...
#define infected(otri)                                                        \
  (((unsigned_long) (otri).tri[6] & (unsigned_long) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((unsigned_long) (sptr) & (unsigned_long) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((unsigned_long) (sptr) & ~ (unsigned_long) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((unsigned_long) (osub).ss | (unsigned_long) (osub).ssorient)
...

{
  ...
  alignptr = (unsigned_long) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (unsigned_long) pool->alignbytes -
     (alignptr % (unsigned_long) pool->alignbytes));
...

{
  ...
  alignptr = (unsigned_long) (pool->pathblock + 1);
...

{
  ...
  alignptr = (unsigned_long) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (unsigned_long) m->triangles.alignbytes -
     (alignptr % (unsigned_long) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (unsigned_long) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (unsigned_long) m->subsegs.alignbytes -
       (alignptr % (unsigned_long) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (unsigned_long) (getblock + 1);
  foundvertex = (char *) (alignptr + (unsigned_long) m->vertices.alignbytes -
                          (alignptr % (unsigned_long) m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
unsigned_long randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
unsigned_long randomnation(choices)
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (unsigned_long) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (unsigned_long) m->triangles.alignbytes -
                         (alignptr %
                          (unsigned_long) m->triangles.alignbytes));
